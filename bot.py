...
async def process_listing(user_id, message):
    ...
    try:
        ...
        result = (
            ...
            f"თავსებადობა:\n{compat_text}\n\n"
            f"სტატუსი: {draft_res}\n\n"
            "ახლა შეამოწმე შენი eBay Seller Hub -> Drafts გვერდი!"
        )
...
async def create_ebay_inventory_draft(listing):
    ...
    # ვიყენებთ სუფთა SKU ფორმატს სპეციალური სიმბოლოების გარეშე
    clean_part = re.sub(r'[^a-zA-Z0-9]', '', listing.get('part_number', 'PART'))
    sku = f"AUTO-{clean_part}-10965"
    ...
    try:
        ...
        if response.status_code in [200, 201, 204]:
            return f"✅ წარმატებით ჩაიწერა დრაფტებში! (SKU: {sku})"
        
        # თუ მაინც შეცდომაა, ამოვიღოთ ზუსტი მესიჯი ლოგისთვის
        try:
            err_data = response.json()
            msg = err_data.get("errors", [{}])[0].get("message", "Unknown REST error")
            return f"eBay შეტყობინება: {msg}"
        except:
            return f"eBay REST კოდი: {response.status_code}"
    except Exception as e:
